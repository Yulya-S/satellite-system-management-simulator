...
func _draw() -> void:
	...
	for i in range(11):
		...
		draw_string(ThemeDB.fallback_font, Vector2(pos_x - 25, 190), str(round((min_x + (x_step * i))* 10) / 10), 1, 50, 9, Color.BLACK)
	for i in range(11):
		...
		draw_string(ThemeDB.fallback_font, Vector2(8, pos_y + 5), str(max_y - (y_step * i)), 1, 50, 10, Color.BLACK)
	if len(x) == 1:
		var x_pos = 51 + (324. * x[0] / max_x)
		var y_pos = 10 + (160. * 1 / max_y)
		draw_circle(Vector2(x_pos, y_pos), 2, Color.FIREBRICK)
	else:
		for i in range(len(x) - 1):
			var x1_pos = 51 + (324 * (x[i] - min_x) / (max_x - min_x))
			var y1_pos = 10 + (160. * i / (max_y - 1))
			var x2_pos = 51 + (324 * (x[i + 1] - min_x) / (max_x - min_x))
			var y2_pos = 10 + (160. * (i + 1) / (max_y - 1))
...
func update_values(new_x: Array):
	x = new_x
	queue_redraw()
